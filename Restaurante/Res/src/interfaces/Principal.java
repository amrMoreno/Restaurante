					...
					if (Productos.TipoProducto.REFRESCO != null) {
						try {
							stmte = c.createStatement();
							 ...
							 while (rst.next()) {
								 Productos Bebida=new Productos(rst.getString("nombre"),TipoProducto.REFRESCO,rst.getString("precio"));
		...
		BComida.addMouseListener(new MouseAdapter() {
			...
			public void mouseClicked(MouseEvent arg0) {
				...
				if (Productos.TipoProducto.COMIDA_PRINCIPAL != null) {
					try {
						stmte = c.createStatement();
						 ResultSet rst = stmte.executeQuery("SELECT * FROM productos WHERE tipoDeProducto = 'COMIDA_PRINCIPAL'");
						
						 while (rst.next()) {
							 Productos comida=new Productos(rst.getString("nombre"),TipoProducto.COMIDA_PRINCIPAL,rst.getString("precio"));
				...
				if (Productos.TipoProducto.COPAS != null) {
					try {
						stmte = c.createStatement();
						 ...
						 while (rst.next()) {
							 Productos comida=new Productos(rst.getString("nombre"),TipoProducto.COPAS,rst.getString("precio"));
				...
				if (Productos.TipoProducto.LICORES != null) {
					try {
						stmte = c.createStatement();
						 ...
						 while (rst.next()) {
							 Productos comida=new Productos(rst.getString("nombre"),TipoProducto.LICORES,rst.getString("precio"));
				...
				if (Productos.TipoProducto.VINOS != null) {
					try {
						stmte = c.createStatement();
						 ...
						 while (rst.next()) {
							 Productos comida=new Productos(rst.getString("nombre"),TipoProducto.VINOS,rst.getString("precio"));
